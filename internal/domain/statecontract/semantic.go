...
func SemanticViolations(snapshot Snapshot) []SemanticViolation {
	if snapshot.Version != 0 && !snapshot.LegacySource() {
		if err := snapshot.ValidateVersion(); err != nil {
			return []SemanticViolation{{Field: "version", Code: SemanticCodeContractVersionMismatch, Reason: err.Error(), MigrationRule: "RUN_VERSIONED_MIGRATION", OperatorGuide: "explicit snapshot migration is required"}}
		}
	}
	violations := []SemanticViolation{}
	...
	activeField, ok := FieldByPath("active_execution")
	if !ok {
	...
	for _, key := range keys {
		...
		if RequiredForStatus(activeField, issue.Status) && !activeMatches {
		...
		if !RequiredForStatus(field, issue.Status) || !CrossedWorkerExecutionBoundary(issue) {
			continue
		}
