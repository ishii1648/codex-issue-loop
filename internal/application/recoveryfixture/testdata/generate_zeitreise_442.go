...
func main() {
	issue, err := os.ReadFile("internal/adapter/state/testdata/zeitreise-442-v0614-full-27-state.json")
	must(err)
	file, err := os.Open("internal/adapter/state/testdata/zeitreise-442-v0614-full-27-events.jsonl")
	must(err)
	defer file.Close()
	...
	must(err)
	must(os.WriteFile("internal/application/recoveryfixture/testdata/zeitreise-442-full-history-v1.json", append(output, '\n'), 0o600))
}
