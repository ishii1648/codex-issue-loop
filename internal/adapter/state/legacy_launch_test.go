...
func TestNormalizeLegacyWorkerLaunchesRestoresOnlyMatchingAnsweredResumeAuthority(t *testing.T) {
	tests := []struct {
		...
		{name: "ordinary retry after historical answer", mutate: func(snapshot *Snapshot) {
...
func TestValidatorRejectsForgedAnsweredResumeLaunchSource(t *testing.T) {
	snapshot := legacyAnsweredLaunchFixture()
	NormalizeLegacyWorkerLaunches(&snapshot)
	snapshot.Issues["1"].LaunchSource = issuedomain.StatusRetryWait
	if err := snapshot.Validate(); err == nil {
		t.Fatal("launch source contradicting answered continuation evidence was accepted")
	}
}
