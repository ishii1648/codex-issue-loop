...
func legacyWorkerLaunchSource(snapshot *Snapshot, issue *Issue) (issuedomain.Status, string, bool) {
	...
	if request == nil || request.ID != checkpoint.RequestID || request.IssueNumber != issue.Number ||
		...
		request.ReleasedExecution.RunID != checkpoint.RunID || request.ReleasedExecution.Generation == 0 ||
		request.ReleasedExecution.Generation > checkpoint.Generation || request.Status != issuedomain.RequestStatusAnswered ||
	...
	if request.ReleasedExecution.Generation == checkpoint.Generation {
