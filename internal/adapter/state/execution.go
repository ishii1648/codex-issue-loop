...
func validateUnstartedConflictLaunch(snapshot *Snapshot, issue *Issue) error {
	return statecontract.ValidateUnstartedConflictLaunch(snapshot, issue)
}
...
func legacyWorkerLaunchSource(snapshot *Snapshot, issue *Issue) (issuedomain.Status, string, bool) {
	return statecontract.LegacyWorkerLaunchSource(snapshot, issue)
}

func validSHA256(value string) bool { return statecontract.ValidSHA256(value) }
