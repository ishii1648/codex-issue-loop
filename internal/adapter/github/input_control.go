...
func (c CLI) SyncInputAcknowledgement(ctx context.Context, cfg config.Config, number int, acknowledgement InputAcknowledgement) error {
	acknowledgement.RequestID = redact.StringWithSecrets(acknowledgement.RequestID, c.Secrets)
...
func (c CLI) syncManagedComment(ctx context.Context, repo string, number int, marker, body string) error {
	...
	legacyMarker := ""
	if strings.HasPrefix(marker, "<!-- codex-issue-loop:input-request:v1 request=") {
		id := strings.TrimSuffix(strings.TrimPrefix(marker, "<!-- codex-issue-loop:input-request:v1 request="), " ")
		legacyMarker = "<!-- codex-issue-loop:request:" + id + " -->"
	}
	for _, comment := range comments {
		if strings.EqualFold(comment.Actor, viewer) && (strings.Contains(comment.Body, marker) ||
			legacyMarker != "" && strings.Contains(comment.Body, legacyMarker)) {
