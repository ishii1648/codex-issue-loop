...
gh issue list --repo "$repository" --label "$label" --json number --jq ".[] | select(.number == $issue_number)" | grep -q "$issue_number"
...
gh pr list --repo "$repository" --head "$branch" --json number --jq ".[0].number" | grep -qx "$pr_number"
