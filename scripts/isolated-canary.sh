...
sed -e "s#owner/repository#$repository_escaped#" -e "s/# repository_id: 123456789/repository_id: $repo_id_escaped/" \
  "$GITHUB_WORKSPACE/.agent-loop.example.yaml" >"$repo_path/.agent-loop.yaml"
...
mkdir -p "$state_root" "$launch_root" "$artifact_dir"
$binary bootstrap-labels --repo "$repo_path" --apply --json >/dev/null
$binary register --repo "$repo_path" --json >/dev/null
...
sequence_one_url=$(gh issue create --repo "$repository" --title "canary completion $GITHUB_RUN_ID" --body "Create canary-sequence-1.txt containing completion, run git status, and return completed." --label codex-loop:ready)
sequence_one=${sequence_one_url##*/}
start_supervisor
wait_issue_status "$sequence_one" completed
stop_idle_supervisor

sequence_two_url=$(gh issue create --repo "$repository" --title "canary needs input $GITHUB_RUN_ID" --body "Before editing, return needs_input asking whether to create canary-sequence-2.txt. After the answer, create it containing resumed and return completed." --label codex-loop:ready)
...
jq -n \
  ...
  '{schema_version:1, repository:$repository, repository_id:$repository_id, sequences:[{id:"claim-worker-publication-checks-terminal",issue:$sequence_one,status:"completed"},{id:"needs-input-answer-resume-terminal",issue:$sequence_two,status:"completed"}],supervisor_restarts:2,webhook_miss_polling_fallback:1,transaction_crash_recovery:1,production_remote_changes:0,production_state_accessed:false,final:{active_workers:$active,active_leases:$active_leases,pending_requests:$pending,orphan_pid_pgid:$orphan_processes,duplicate_prs:$duplicate_prs,duplicate_comment_markers:$duplicate_markers}}' \
