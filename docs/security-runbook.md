...
4. `.agent-loop.yaml`の`worker.sandbox`を`workspace-write`（Codex以外のbackendの調査専用なら`read-only`）にする。`danger-full-access`は設定検証で拒否される。workerの追加権限要求はCodexの自動レビューへ送る。command networkは既定無効とし、local HTTP/CDPが必要なrepositoryだけ[localhost-only command network](localhost-network.md)の固定policyと実機負テストを適用する。`sandbox_workspace_write.network_access=true`だけを設定してはならない。
