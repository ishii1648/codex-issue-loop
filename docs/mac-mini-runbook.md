...
v0.6.14で一度resumeし、spawn前に`saved workspace provenance is missing`で再blockされたrecordもstateを編集せず同じコマンドを使う。自動回復されるのは、`status=blocked`、`workspace=null`、active `environment_resume.status=requested|github_synced`、`blocked_cause`がsupervisor / `worker_workspace` / non-resumableの厳密なmissing-provenance reason、PID/PGID不在、同じrunとactive lease owner generationを保持する場合だけである。さらにevent historyの同じresume ID、`previous_reason`、base/current-base SHA、worktree/branch、park/lease owner、resume同期、validator rejection、blocked同期と、GitHubの`blocked` label・resume/failure markerが一致しなければ拒否される。workspace mismatch、symlink、branch/repository mismatch、別supervisor error、manual/security/publication blockは対象外である。成功後は`environment_resume_recovered`の`interrupted_workspace_recovery=true`と`workspace_recovery`、不変のresume ID/lease ownerを確認する。
