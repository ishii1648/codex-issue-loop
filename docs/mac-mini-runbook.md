...
v0.6.14で一度resumeし、spawn前に`saved workspace provenance is missing`で再blockされたrecordもstateを編集せず同じコマンドを使う。実際のlegacy lease recovery recordは`status=blocked`、`workspace=null`、`environment_resume.status=running`、resource parkなし、active leaseありである。CLIは同一Issue/run/resume ID、保存worktree/branch/session、base/current-base SHA、current leaseとその直前generationの`lease_reserved`、初回workerとlegacy block/reconciliation、`legacy_lease_recovered=true`かつowner/slotなしのrequest、resume IDなし→ありの2段階GitHub sync、resume worker、validator rejection、blocked同期の完全な順序を一体で照合する。GitHubの`blocked` label・同じresume/failure markerも必須である。status `running`、owner/slot欠損、2段階同期を個別には許可しない。成功後は`environment_resume_recovered`の`interrupted_workspace_recovery=true`、`workspace_recovery`、不変のresume ID/run/worktree/branch/session/SHA、およびcurrentから1つ進んだ`lease_owner.generation`を確認する。
