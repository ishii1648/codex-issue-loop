...
- 公開repositoryではGitHubのIssue作成policyを`Collaborators only`にし、ready labelの付与権限も信頼済みcollaboratorへ限定する。Issue本文はworkerのnetwork、browser、downloadその他の起動権限を指定・拡張しない。
