...
ただしv0.6.14で明示resumeした後、spawn前validatorの`Workspace`欠損だけで停止したrecordは限定例外とする。current stateが`status=blocked`、`Workspace=nil`、`environment_resume.status=requested|github_synced`、supervisor-origin / `worker_workspace` / non-resumableで、reasonが保存worktreeに対する厳密な`saved workspace provenance is missing`、PID/PGID不在、active lease ownerが同じrunとgenerationを保持する場合に限り候補になる。authorityはerror文字列ではなく、同じresume ID・run・`PreviousReason`・base/current-base SHA・park ID・lease owner/slotを持つ`environment_resume_requested`、resume marker同期、同じworktree/branchと全境界checkを通過した`worker_started` / `worker_workspace_rejected`、blocked同期の順序付きdurable event chainである。GitHubはsupervisor-owned `blocked`だけ、同じresume markerとworkspace failure markerを保持しなければならない。chainの欠損・重複・順序不正・改変、別cause、既存Workspace、active worker、別lease generation、base移動、manual/security/publication recoveryは拒否する。

この限定例外も通常のmissing-provenance recoveryと同じ`ValidateLaunch`、完全一致検査、欠損時だけのatomic backfill、`workspace_recovery`監査payloadを使う。成功transactionは既存resume ID、lease owner/generation、run/worktree/branch/session、original/current base SHAを変えず`environment_resume_pending`へ戻し、current supervisor safety causeは監査payloadへ保存したうえで、検証済み`EnvironmentResume.PreviousReason`をworker-origin / environment / resumable causeへ復元する。並行実行はstate revisionで一方だけがcommitし、restart/retryは保存Workspaceを再検証するidempotent pathへ収束する。worker continuationにはsupervisor validator errorではなく同じ`PreviousReason`を渡し、dirty/staged/untracked差分とHEADを変更しない。
