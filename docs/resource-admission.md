...
| terminal `blocked` / `failed` | checkpoint | PID/PGID不在を確認し、active leaseを`continuation_checkpoint.original_execution_lease`へ移してadmission対象から外す |
| `issue resolve`で継続 | 新generationで再取得 | 保存checkpointと全active lease、worker slotを同じstate transactionで再検証する |
...
| retry上限到達、`failed`、`blocked` | 解放 | terminal遷移と同時にhard leaseをcheckpointへ移しcapacityを解放する |
...
workerまたはlifecycle stageがterminal `blocked` / `failed`へ遷移すると、同じdurable transactionでactive leaseをgeneric checkpointへ移す。checkpointは元owner generation、slot、declared/resolved/actual resources、base SHA、workspace、session、stage、result digest、evidenceを保持するが、admissionのactive lease集合には含めない。run、worktree、branch、dirty changes、answers、attempt/continuationは変更しない。
...
`issue plan`はcheckpointと現在のprocess/git/GitHubをread-only評価する。`issue resolve`は同一revision、run/worktree/branch/session、保存時と現在のworktree content SHA-256、GitHub Issue/PR/label、pending request不在、保存base SHAのHEAD祖先性、dirty/unpushed状態、空worker slot、全active leaseとの非競合をtransaction内で再検証する。競合するIssueのleaseは奪わず副作用なく拒否する。成功時は`lease_generation`を1回だけ増やした新ownerをcheckpoint、audit event、active leaseへ同時保存し、再実行は同じresolutionへ収束する。
