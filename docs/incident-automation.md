...
3. `expected_transient`とcircuit由来のepisodeはAIへ渡さない。circuit由来のepisodeはIssue判定からも除外し、decisionsには`skipped` / `automation_circuit_episode`を記録する。それ以外はsanitized evidenceだけをowner-onlyの一時directoryで`codex exec --sandbox read-only --ephemeral --ignore-user-config --ignore-rules --skip-git-repo-check`へ渡し、versioned output schemaを検証する。repository checkout、user MCP/plugin、session rolloutはanalysis contextへ含めない。
