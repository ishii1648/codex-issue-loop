...
GitHub Issueがauthoritativeに`NOT_PLANNED`でcloseされている場合、terminal `blocked` / `failed`かつPID/PGID、pending request、対象Issue自身のroot `active_execution`がなく、保存済みPR identityが一意に一致するときだけ共通reconcilerが`canceled`へ遷移する。別Issueの`active_execution`は所有権を照合して保持する。worktreeと成果物は変更せず、`issue_canceled`をsnapshotと同じtransactionへ記録する。
