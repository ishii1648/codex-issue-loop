...
将来のschema v3では、同一repository内並列実行のresource claimとして`area:<resource>` labelを使用し、configのresource definitionから不足labelをbootstrap対象へ加える。prefix、正規化、不正・未知labelの縮退規則は[Resource admission契約](resource-admission.md)を正本とする。現行schema v2のbootstrap対象には含めない。
