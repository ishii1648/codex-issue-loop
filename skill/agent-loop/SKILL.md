...
For a worker environment block, require `blocked_cause.origin=worker`, `blocked_cause.kind=environment`, and `blocked_cause.resumable=true`. A pre-feature legacy record is eligible only when the CLI recognizes its supervisor-generated `failure_kind=issue` and `worker blocked` marker; let the CLI normalize provenance and conservatively reacquire `repo:*`, never edit state to make it eligible. Explain and obtain the operator's explicit confirmation that the external prerequisite is resolved, confirm no active worker and consistent saved run/worktree/branch/resource lease/GitHub PR state, then use `agent-loop resume-blocked --repo <path> --issue <number> --confirm-prerequisite-resolved --json`. Do not use this command for Pull Request conflict recovery, manual exclusions, security blocks, running/completed Issues, closed-without-merge state, or unrecognized legacy records. Never edit supervisor labels or durable state manually; preserve dirty changes, session, answers, and resource metadata.
