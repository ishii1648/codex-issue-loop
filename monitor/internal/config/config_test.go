...
func TestLoadSupportsMultipleIsolatedRepositories(t *testing.T) {
	...
	if cfg.Repositories[0].DoneLabel != "codex-loop:done" || cfg.Repositories[1].DoneLabel != "queue:done" {
		t.Fatalf("done labels=%+v", cfg.Repositories)
	}
...
func TestLoadRejectsUnknownFieldsAndRelativeState(t *testing.T) {
	for _, body := range []string{
		`version: 1
repositories: [{name: owner/repo, done_label: " "}]
`,
