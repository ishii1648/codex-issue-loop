...
for index, repo in enumerate(repos):
    ...
    for i, state in enumerate(states):
        ...
        intervals.append({'id': repo+':'+str(i), 'repository': repo, 'status': state, 'started_at': stamp(start), 'ended_at': stamp(start+timedelta(hours=1)), 'reason': 'queue progress deadline exceeded' if state == 'DOWN' else ''})
    (directory / 'intervals.jsonl').write_text(''.join(json.dumps(v)+'\n' for v in intervals))
    (directory / 'current.json').write_text(json.dumps({'schema_version':1,'repository':repo,'last_observation_at':stamp(observed),'queue_deadline':stamp(observed+timedelta(minutes=10)) if status=='HEALTHY' else stamp(observed),'queue':([{'number':295,'phase':'running','phase_since':stamp(observed-timedelta(hours=1)),'deadline':stamp(observed)}] if status in ['HEALTHY','DOWN'] else []),'current':{'id':repo+':current','repository':repo,'status':status,'started_at':stamp(observed),'reason':'queue progress deadline exceeded' if status == 'DOWN' else ''}}))
