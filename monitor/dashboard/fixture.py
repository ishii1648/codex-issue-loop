...
for index, repo in enumerate(repos):
    ...
    for i, state in enumerate(states):
        ...
        intervals.append({'decision_version':2,'id': repo+':'+str(i), 'repository': repo, 'status': state, 'started_at': stamp(start), 'ended_at': stamp(start+timedelta(hours=1)), 'reason': 'queue progress deadline exceeded' if state == 'DOWN' else ''})
    (directory / 'intervals.jsonl').write_text(''.join(json.dumps(v)+'\n' for v in intervals))
    (directory / 'current.json').write_text(json.dumps({'schema_version':1,'decision_version':2,'decision_since':stamp(observed-timedelta(hours=4)),'repository':repo,'last_observation_at':stamp(observed),'queue_deadline':stamp(observed+timedelta(minutes=10)) if status=='HEALTHY' else stamp(observed),'queue':([{'number':295,'phase':('ready' if status=='DOWN' else 'running'),'phase_since':stamp(observed-timedelta(hours=1)),'deadline':stamp(observed)}] if status in ['HEALTHY','DOWN'] else []),'current':{'decision_version':2,'id':repo+':current','repository':repo,'status':status,'started_at':stamp(observed),'reason':'queue progress deadline exceeded' if status == 'DOWN' else ''}}))
